...
Deno.test({
  ...
  fn: testBody(async () => {
    ...
    assert(response.body.error, 'Something went wrong: {"error":"Unable to load a key FEATURE_NAME"}');
  }),
});
...
Deno.test({
  ...
  fn: testBody(async () => {
    ...
    assertObjectMatch(response.body, { error: 'Invalid request body' });
  }),
});
...
Deno.test({
  ...
  fn: testBody(async () => {
    ...
    assertObjectMatch(response.body, { error: 'Invalid feature input. Cause: it is required.' });
  }),
});
